...
const ChallanHead = () => {
  ...
  const fetchHeads = async () => {
    try {
      ...
      setHeads(res.data.data.map(applyDisplayFields));
    } catch (error) {
  ...
  const handleToggleStatus = useCallback(async (headId, isActive) => {
    ...
    try {
      await deleteChallanHead(headId);
      setHeads((prev) =>
  ...
  const handleSubmit = async (formData) => {
    ...
    const payload = {
      ...
      subMajorParentCode: formData.majorHeadCode || "0",

      minorHeadParentCode: formData.subMajorCode || "0",

      subHeadParentCode: formData.minorHeadCode || "0",

      subSubHeadParentCode: formData.subHeadCode || "0",

      detailHeadParentCode: formData.subSubHeadCode || "0",
    };
    ...
    try {
      ...
      if (editingHead) {
        res = await updateChallanHead(editingHead.id, payload);

        const updated = applyDisplayFields(res.data.data);
        ...
        res = await createChallanHead(payload);

        const newHead = applyDisplayFields(res.data.data);
