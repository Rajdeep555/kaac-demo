...
const State = () => {
  ...
  const fetchHeads = async () => {
    try {
      setLoading(true);
      const res = await getHeads("STATE");
  ...
  const handleSubmit = async (formData) => {
    setSaving(true);
    setFormError("");
    const payload = { ...formData, sector: "STATE" }; // ✅ STATE sector

    try {
      let res;
      if (editingHead) {
        res = await updateHead(editingHead.id, payload);
        // ✅ FIX: re-apply display fields + use res.data.head (singular)
  ...
  const columns = [
    ...
    {
      // ✅ FIX: actions column was missing render — buttons were never shown
      key: "actions",
      label: "Actions",
      render: (_, row) => {
  ...
  return (
    <>
      <div className="p-6 space-y-6">
        <h1 className="font-unbounded text-3xl">State Head Management</h1>
        ...
        {loading ? (
          ...
          <DataTable
            ...
            statusKey="isActive"
            loading={loading}
            downloadFileName="state-heads"
            printTitle="State Head Report"
            actionSlot={
              <TableButton
                name="Add New Head"
                onClick={() => {
